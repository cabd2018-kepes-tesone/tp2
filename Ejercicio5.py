...
data_sql = data_rdd.map(lambda l: l.split("\t")) \
    .map(lambda d: Row(grupo=int(d[3])/franja))

data_df = sqlContext.createDataFrame(data_sql)

data_df.registerTempTable('trafico')

consulta = "SELECT grupo*%d AS limite_inferior, grupo*%d+%d AS limite_superior, COUNT(*) FROM trafico GROUP BY grupo ORDER BY grupo" % (franja, franja, franja)
